...
void aequatio::log::Log(unsigned int type, std::string msg, std::string func,
                        ...) {
  std::string str;
  ...
  char* aequatioted_string = new char[buff_size];
  vsprintf(aequatioted_string, msg.c_str(), args);
  va_end(buff_args);
  va_end(args);
  str = '[' + std::string(aequatioted_string) + ']';
...
int aequatio::log::GetLogSize() { return global_logs.size(); }

void aequatio::log::ClearLogs() { global_logs.clear(); }

std::vector<std::string> aequatio::log::GetLogs() {
...
std::string aequatio::log::GetLog(unsigned int type) {
  std::string entry;
...
std::pair<unsigned int, std::string> aequatio::log::FGetLog(unsigned int type) {
...
std::string aequatio::log::IGetLog(int index) {
  std::string entry;
...
std::pair<unsigned int, std::string> aequatio::log::IFGetLog(int index) {
  std::pair<unsigned int, std::string> entry;
  if (index >= 0 && (size_t)index < global_logs.size()) {
...
std::vector<std::string> aequatio::log::VGetLog(int start, int end) {
  std::vector<std::string> entries;
...
std::vector<std::pair<unsigned int, std::string>> aequatio::log::VFGetLog(
    int start, int end) {
...
void aequatio::log::SetLogHandle(
    void (*handle)(std::pair<unsigned int, std::string>)) {
  log_handle_full = handle;
}

void aequatio::log::SetLogHandle(void (*handle)(std::string)) {
  log_handle = handle;
}

void aequatio::log::SetLogOption(unsigned int option, int setting) {
...
std::string aequatio::log::GetTypeStr(unsigned int type) {
  std::string str;
  if (type == FATAL) {
    str = "FATAL";
  } else if (type == ERROR) {
...
void aequatio::log::SaveLog(std::string file) {
